...
switch ($_GET["op"]){
	...
	case 'listar':
		$rspta=$usuario->listar();
 		...
 		while ($reg=$rspta->fetch_object()){
 			$data[]=array(
 				"0"=>($reg->condicion)?'<button class="btn btn-warning" onclick="mostrar('.$reg->idusuario.')"><i class="fas fa-pencil-alt"></i></button>'.
    ...
    case 'permisos':
        ...
        while ($per = $marcados->fetch_object())
			{
                //va agregando los permisos 
				array_push($valores, $per->idpermiso);
			}
            while ($reg = $rspta->fetch_object())
				{
                    //pregunta si el idpermiso se encuentra en la tabla de permisos

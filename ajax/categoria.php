...
switch ($_GET["op"]) {
    ...
    case 'listar':
        ...
        while ($resp = $respuesta->fetch_object()) {
            ...
            $data[] = array(
                // si la categoria está activada, entonces 
                "0" => ($resp->condicion) ? '<button class="btn btn-warning" onclick="mostrar(' . $resp->idcategoria . ')"><i class="fas fa-pencil-alt"></i></button>' .
                    ' <button class="btn btn-danger" onclick="desactivar(' . $resp->idcategoria . ')"><i class="fas fa-times"></i></button>' :
                    '<button class="btn btn-warning" onclick="mostrar(' . $resp->idcategoria . ')"><i class="fas fa-pencil-alt"></i></button>' .
                    ' <button class="btn btn-primary" onclick="activar(' . $resp->idcategoria . ')"><i class="fa fa-check"></i></button>',
                "1" => $resp->nombre,
                "2" => $resp->descripcion,
                //cambiar el color del estado de acuerdo a la condición
